...
async function main() {
    ...
    for (const plan of plansData) {
        let existing = await db_1.prisma.platformPlan.findUnique({ where: { code: plan.code } });
        if (!existing) {
            existing = await db_1.prisma.platformPlan.create({ data: plan });
            console.log(`✅ Seeded Plan: ${plan.name}`);
        }
        if (plan.code === 'STARTER')
            starterPlan = existing;
    }
    // 3. Seed Platform Setting
    const setting = await db_1.prisma.platformSetting.findUnique({ where: { id: 'platform' } });
    if (!setting) {
        await db_1.prisma.platformSetting.create({
            data: {
                id: 'platform',
                otpEnabled: true,
    ...
    const sampleRequests = [
        {
            ...
            status: client_1.OwnerRequestStatus.PENDING,
        },
        {
    ...
    for (const req of sampleRequests) {
        const existing = await db_1.prisma.ownerRequest.findUnique({ where: { email: req.email } });
        if (!existing) {
            await db_1.prisma.ownerRequest.create({ data: req });
    ...
    if (!property) {
        property = await db_1.prisma.property.create({
            data: {
                ownerId: owner.id,
                name: 'Sunshine Luxury PG',
                slug: 'sunshine-luxury-pg-koramangala',
                type: client_1.PropertyType.BOYS_PG,
            ...
            for (let r = 1; r <= 3; r++) {
                ...
                const room = await db_1.prisma.room.create({
                    data: {
                        floorId: floor.id,
                        roomNumber: roomNum,
                        type: client_1.RoomType.DOUBLE_SHARING,
                        monthlyRent: 8500,
                    },
                });
                // 2 Beds per room
