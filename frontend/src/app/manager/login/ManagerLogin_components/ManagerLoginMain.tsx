...
export function ManagerLoginMain() {
  ...
  useEffect(() => {
    ...
    if (isDemo || emailParam) {
      setEmail(emailParam || 'demo.manager@smartpg.com');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await api.login({ email, password, expectedRole: 'manager' });
      setSession(user);
    ...
    <div className="home-theme min-h-screen flex bg-[var(--bg-light)] font-sans">
        ...
        <div className="w-full max-w-md mx-auto mt-16">
          ...
          <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-between">
            <div className="text-xs text-amber-800">
              <span className="font-bold">Trying the demo?</span> Use demo manager credentials.
            </div>
            <button
              type="button"
              onClick={() => {
                setEmail('demo.manager@smartpg.com');
                setPassword('Demo@123');
              }}
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-[var(--primary-navy)] items-center justify-center">
        ...
        <div className="relative z-10 p-12 max-w-lg text-center text-white">
          <h3 className="text-4xl font-bold mb-4 leading-tight">Manager Portal</h3>
