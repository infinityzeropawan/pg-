...
import type { MaintenanceTask } from '../ManagerDailyOperations_types/DailyOperations.types';
...
export function ManagerDailyOperationsMaintenance({ tasks, onMarkResolved }: Props) {
  return (
    <div className="bg-card border border-border rounded-[var(--radius-lg)] p-5 h-full">
      <div className="flex items-center justify-between mb-4 border-b border-border pb-3">
        <h2 className="text-lg font-black text-primary">Maintenance Tasks</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="text-xs uppercase text-secondary bg-input/50">
            <tr>
              <th className="px-3 py-2 rounded-l-md">Issue</th>
          ...
          <tbody className="divide-y divide-border">
            {tasks.map(task => (
              <tr key={task.id} className="hover:bg-input/30 transition-colors">
                <td className="px-3 py-3 font-medium text-primary">{task.title}</td>
                <td className="px-3 py-3 text-secondary">{task.location}</td>
                <td className="px-3 py-3">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                    task.priority === 'Urgent' ? 'bg-danger-bg text-danger' : 'bg-theme-primary/10 text-theme-primary'
                  }`}>
                    {task.priority}
                  </span>
                </td>
                <td className="px-3 py-3">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${
                    task.status === 'Resolved' ? 'bg-success-bg text-success' :
                ...
                <td className="px-3 py-3 text-right">
                  {task.status !== 'Resolved' ? (
                    <button 
                      onClick={() => onMarkResolved(task.id)}
            ...
            {tasks.length === 0 && (
              <tr>
                <td colSpan={5} className="text-center py-6 text-secondary text-sm">No maintenance tasks.</td>
              </tr>
            )}
          </tbody>
        </table>
