...
// RESPONSIBILITY: First-login password change for the Student portal.
// Reached when the session has `mustChangePassword` set (see StudentRequireStudent).
    ...
    <div className="min-h-screen flex items-center justify-center bg-page p-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-sm text-secondary mb-6">
          For security, you must replace your temporary password before using the Student Portal.
        </p>

        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-[var(--radius-lg)] p-6 space-y-5 shadow-sm">
          <div>
            <label htmlFor="new-password" className="block text-sm font-bold text-secondary uppercase mb-2">New Password</label>
          ...
          <div>
            <label htmlFor="confirm-password" className="block text-sm font-bold text-secondary uppercase mb-2">Confirm Password</label>
            <input
              id="confirm-password"
              type={showPassword ? 'text' : 'password'}
              required
